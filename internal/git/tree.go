...
func getTreeSignature(dir string, ref string) (*attest.Signature, error) {
	...
	for _, line := range strings.Split(msg, "\n") {
		...
		if len(elts) == 2 && strings.HasPrefix(elts[0], signatureMarker) {
			sig, err := attest.DecodeSignature([]byte(elts[1]))
			if err != nil {
				return nil, err
			}
			return sig, nil
...
func SignTree(dir string, ref string) ([]byte, *attest.Signature, error) {
...
func readAllowedSigners(dir string) ([]*Signer, error) {
	allowedSigners, err := ConfigGet(dir, "attest", "allowedSignersFile", "")
	if err != nil {
		return nil, err
	}

	if allowedSigners == "" {
		return nil, fmt.Errorf("attest.allowedSignersFile must be configured in .gitconfig")
	}

	path, err := fsutil.ExpandPath(allowedSigners)
	if err != nil {
		return nil, err
	}
	...
	for _, line := range strings.Split(string(data), "\n") {
		...
		if line != "" && !strings.HasPrefix(line, "#") {
			...
			pubKey, err := attest.DecodePublicKey([]byte(elts[1]))
			if err != nil {
				return nil, err
			}

			slog.Debug("trust signer", "uid", elts[0], "pubkey", pubKey)
...
func readPrivateKey(dir string) (*attest.PrivateKey, error) {
	...
	path, err := fsutil.ExpandPath(privFile)
	if err != nil {
		return nil, err
	}

	privKey, err := attest.ReadPrivateKey(path)
	if err != nil {
		return nil, err
	}
